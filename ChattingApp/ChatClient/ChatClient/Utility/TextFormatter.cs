    ...
    class TextFormatter
    {
        ...
        public static TextBlock FormatText(ChatMessage message)
        {
            ...
            for (int index = 0; index < textToFormat.Length - 1;)
            {
                ...
                {
                    makeBold(ref formattedText, ref textToFormat, ref index, ref sectionToFormat, ref currentSection, ref isBold);
                }
                else if (currentSection.ToString().Equals(" " + italicSymbol))
                {
                    makeItalic(ref formattedText, ref textToFormat, ref index, ref sectionToFormat, ref currentSection, ref isItalic);
                }
                else if (currentSection.ToString().Equals(" " + underlineSymbol))
                {
                    makeUnderline(ref formattedText, ref textToFormat, ref index, ref sectionToFormat, ref currentSection, ref isUnderlined);
                }
                else
                {
        ...
        private static void makeBold(ref TextBlock formattedText, ref string textToFormat, ref int index, ref StringBuilder sectionToFormat,
            ref StringBuilder currentSection, ref bool isBold)
        {
            formattedText.Inlines.Add(new Run(textToFormat[index].ToString()));
        ...
        private static void makeItalic(ref TextBlock formattedText, ref string textToFormat, ref int index, ref StringBuilder sectionToFormat,
            ref StringBuilder currentSection, ref bool isItalic)
        {
            formattedText.Inlines.Add(new Run(textToFormat[index].ToString()));
        ...
        private static void makeUnderline(ref TextBlock formattedText, ref string textToFormat, ref int index, ref StringBuilder sectionToFormat,
            ref StringBuilder currentSection, ref bool isUnderlined)
        {
            formattedText.Inlines.Add(new Run(textToFormat[index].ToString()));
            ...
            for (int index = 0; index < text.Length - 1; ++index)
            {
                if (isStriked)
                {
                    if (text.ToString().Substring(index, 2).Equals(strikeSymbol + " "))
                    {
                        ...
                        {
                            if(strikeCharConverter.ContainsKey(text[index2]))
                            {
                                text.Replace(text[index2].ToString(), strikeCharConverter[text[index2]], index2, 1);
                            }
                            else
                            {
                                text.Replace(text[index2].ToString(), "??", index2, 1);
                            }
                            ++endingPosition;
                            index += 2;
                        }
